...
class VaultManager: ObservableObject, SyncServiceDelegate {
    ...
    private func loadEntries() throws {
        ...
        for row in try db.prepare(query) {
            ...
            mutableEntry.createdAt = row[createdAt]
            mutableEntry.modifiedAt = row[modifiedAt]
    ...
    private func loadCategories() throws {
        ...
        for row in try db.prepare(categoriesTable) {
            loadedCategories.append(Category(id: UUID(uuidString: row[catId])!, name: row[catName], icon: row[catIcon], color: row[catColor]))
        }
    ...
    private func storeSalt(_ salt: Data) throws {
        ...
        guard status == errSecSuccess else { throw VaultError.keychainError(status) }
    }
    ...
    func syncServiceDidConnect(_ service: SyncService) { syncStatus = "Connected to desktop" }
    func syncServiceDidDisconnect(_ service: SyncService) { syncStatus = "Disconnected" }
            ...
            try db.transaction {
                ...
                for incoming in incomingEntries {
                    ...
                    if let localRow = try db.pluck(query) {
                        if incoming.modifiedAt >= localRow[modifiedAt] {
                            let newEncryptedPassword = (incoming.encryptedPassword.isEmpty && !localRow[encryptedPassword].isEmpty) ? localRow[encryptedPassword] : incoming.encryptedPassword
                            let newEncryptedNotes = (incoming.encryptedNotes == nil && localRow[encryptedNotes] != nil) ? localRow[encryptedNotes] : incoming.encryptedNotes
                ...
                DispatchQueue.main.async {
                    ...
                    self.syncStatus = "Sync complete: \(insertedCount) added, \(updatedCount) updated"
                }
            }
        } catch {
            DispatchQueue.main.async { self.syncStatus = "Sync failed: \(error.localizedDescription)" }
        }
    }
    
    func syncService(_ service: SyncService, didEncounterError error: Error) { syncStatus = "Sync error: \(error.localizedDescription)" }
